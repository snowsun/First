...
// 	out.write(result);
	...
	String wks = new DB_config().fetch_config()[7];
	int  N = wks.split("@").length;
	String unique[] = new String[N];
	for(int i=0 ; i<N ; i++){
		unique[i] = wks.split("@")[i] ; 
	}
	...
	for(int r=0 ; r<N ; r++){
		for(int i=0 ; !cinfo[i][0].equals("over") ; i++){
			if(cinfo[i][5].equals("星期"+unique[r])){
				name = name + "+-+"+cinfo[i][1];
				teacher = teacher + "+-+" +cinfo[i][4];
				lab = lab + "+-+" +cinfo[i][3];
				T1 = T1 + "+-+" + cinfo[i][9];
		...
		re[r] = turns+"*"+name+"*"+teacher+"*"+lab+"*"+T1+"*"+T2+"*"+T3+"*"+T4+"*"+T5;
		name ="实验名称";
		teacher = "指导教师";
		lab = "实验室";
		T1 = "第一轮";
